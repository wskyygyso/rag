...
- 每完成一个阶段性事项，先完成测试、验证和 `progress.md` 更新；
- 默认不主动执行 Git commit，先等待用户审阅；
- 只有用户明确确认“可以提交”或表达等价意图后，才创建中文 commit；
- commit 说明必须使用中文，明确记录本次推进或修改内容；
- 未经确认的改动保持在工作区，不能为了提交而覆盖或丢弃。
